...
ただ、しばらく利用してみて色々な不満がでてきます。(どんなライブラリやツールでもそうですが)

その不満の中でも、Openapiが分割管理出来ない点が特に不満を感じます。

分割管理が行えないと複数チームでの運用時に、コンフリクトが発生する頻度が高く運用を工夫する必要がでてきます。

昔のようなDBマイグレーションのようなブランチ運用をしたくない場合、コンフリクトが発生しやすいのは大変煩わしいです。
